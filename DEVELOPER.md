...
- **`VsCodeLmHandler`** wraps the LM API. `getClient(cfg)` calls `vscode.lm.selectChatModels({ vendor: "copilot" })` behind a **10-second timeout guard**, then always picks the model configured in `vsllmServer.model` (falling back to the first available one). It no longer takes a requested model id — the `model` field sent by the client is ignored entirely, so a client can't switch the active model by asking for a different (even valid) name. `handleChatCompletions()` reports the response's `model` as the configured `cfg.modelId` (or `"vsllm-copilot"`), never the client's requested value.
...
Then run `npm run watch` in a terminal and launch. In the Extension Development Host, open the VSLLM Server sidebar and flip the **Server** switch on. Extension logs go to the *Debug Console*; request traffic is inspected through the Traffic Monitor panel; the panel's own logs appear in the webview devtools (**Developer: Open Webview Developer Tools**).
